...
function create_material(useSkinning, useAlpha, colSkin, colCloth, tex1, tex2) {
  ...
  material = new THREE.ShaderMaterial( {
    uniforms: uniforms,
    vertexShader: useSkinning ? NrgShader.vrtxShdrSkinning : NrgShader.vrtxShdrBasic,
...
function loadModel(data, useAlpha = false, ignoreBoneAni = false) {
  ...
  const promises = [];
  promises.push(AsyncLoader.LoadFBX("char/character/fbx/" + data.fbx + ".fbx"));
  promises.push(AsyncLoader.LoadTexture("char/character/textures/" + data.texMain + ".png"));
  if(data.texMask) {
    promises.push(AsyncLoader.LoadTexture("char/character/textures/" + data.texMask + ".png"));
  } else {
  ...
  return Promise.all(promises).then(result => {
    ...
    fbx.traverse(child => {
      ...
      if(result.length > 2) {
        child.material = create_material(isSkinMesh && !ignoreBoneAni, useAlpha, data.colSkin, data.colCloth, result[1], result[2]);
      } else {
        child.material = create_material(isSkinMesh && !ignoreBoneAni, useAlpha, data.colSkin, data.colCloth, result[1], null);
      }
    });
    fbx.name = data.fbx;
    return fbx;
...
function set_head_parts_offset_single(objt, dmName, partsData, rotDegee)
{
  ...
  let dm = null;
  objt.traverse((child) => { if (child.name.match(dmName)) { dm = child; } });
  if(dm === null) {
...
function set_head_parts_offset_pair(root, lobj, robj, dmName, partsData, distOffset)
{
  ...
  let lDm = null;
  let rDm = null;
  lobj.traverse((child) => { if (child.name.match(dmName)) { lDm = child; } });
  robj.traverse((child) => { if (child.name.match(dmName)) { rDm = child; } });
...
function create_head_parts(result, ridx, loadata, avatar) {
  ...
  if(loadata.mustache && ridx < result.length && result[ridx]) {
    const objt = result[ridx];
    objt.name = 'Mustache';
    set_head_parts_offset_single(objt, 'mus_dm', avatar.avatarMustacheData, -90);
  ...
  if(loadata.eye && ridx < result.length && result[ridx]) {
    ...
    set_head_parts_offset_pair(root, lobj, robj, 'eye_dm', avatar.avatarEyeData, 3);
  ...
  if(loadata.eyebrow && ridx < result.length && result[ridx]) {
    ...
    set_head_parts_offset_pair(root, lobj, robj, 'eyebrow_dm', avatar.avatarEyeBrowData, 1);
    //console.log('head parts:', ridx, root);
    ++ridx;
  }
  ...
  if(loadata.glass && ridx < result.length && result[ridx]) {
    ...
    let lDm = null;
    let rDm = null;
    lobj.traverse((child) => { if (child.name.match('glasses_dm')) { lDm = child; } });
    robj.traverse((child) => { if (child.name.match('glasses_dm')) { rDm = child; } });
...
function loadBodyMaster(loadata, avatar) {
  ...
  // 반드시 있어야 하는 오브젝트들
  promises.push(AsyncLoader.LoadFBX("char/character/fbx/body_master/body_master.fbx"));
...
function LoadCharacter(json_obj, cmodel) {
  ...
  const promises = [];
  promises.push(AsyncLoader.LoadFBX("char/character/ani/@idle_01.fbx"));
  ...
  if(loadata.top) { promises.push(loadModel(loadata.top)); }
  if(loadata.bottom) { promises.push(loadModel(loadata.bottom)); }
  if(loadata.shoes) { promises.push(loadModel(loadata.shoes)); }
