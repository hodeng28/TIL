...
const Follow: NextPageWithLayout = () => {
  ...
  const { userFollowingId, nickname } = router.query;
  ...
  const handleClickTab = () => {
    if (Reflect.has(router.query, 'id')) {
      router.push({
        pathname: `/follower/${userFollowingId}`,
        query: {
          nickname,
          id: router.query.id
        }
    ...
    router.push(`/follower/${userFollowingId}?nickname=${nickname}`);
  };
  ...
  return (
    <FollowContainer ref={listRef} onScroll={handleScroll}>
      <HeaderWrapper>
        <Header
          pageName={nickname}
          options={{ back: true }}
        ...
        <TabsWrapper>
          <StyledTabs>
            <StyledTab onClick={handleClickTab}>팔로워</StyledTab>
            <StyledTabActive>팔로잉</StyledTabActive>
          </StyledTabs>
        </TabsWrapper>
      </HeaderWrapper>
      <ListWrapper>
        {followList?.length !== 0 ? (
          ...
          <NoFollowWrapper>
            <NoPersonOutlineIcon />
            <NoFollowTypography>팔로잉하는 사람이 없습니다.</NoFollowTypography>
          </NoFollowWrapper>
        )}
      </ListWrapper>
    </FollowContainer>
